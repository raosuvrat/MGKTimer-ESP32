...
void init_beam(beam_t *beam) {
  ...
  switch (beam_ptr->mode) {
    case LASER_IR_RECV:
      ...
      LOGF("IR pulse train timer enabled: %d\n",
           timerAlarmEnabled(ir_pulse_train_timer));
  ...
  LOGF("Poll beam timer enabled: %d\n", timerAlarmEnabled(poll_beam_timer));
